...
Begin VB.Form frmGerenciarNFes 
   ...
   Begin VB.ComboBox cmbSituacao 
      Height          =   315
      ItemData        =   "frmGerenciarNFes.frx":0000
      Left            =   4800
      List            =   "frmGerenciarNFes.frx":0013
...
Dim NFeCaminhoRetorno As String
Dim NFeCaminhoErro As String
Dim NFeCaminhoEnvio As String
Dim NFeCaminhoEnviadas As String
Dim NFeCaminhoTemp As String
...
''   rsNFe.Open "Select * from NFe Order By Numero", cnSistema, adOpenForwardOnly, adLockOptimistic, 1
   ...
   lvwMensagens.ColumnHeaders.Add , , "Mensagem", 6000
   lvwMensagens.ColumnHeaders.Add , , "Arquivo", 5000
...
'         If Trim(sMotivo) <> "" Then
            ...
            Print #1, "<consSitNFe xmlns=" & Chr(34) & "http://www.portalfiscal.inf.br/nfe" & Chr(34) & " versao=" & Chr(34) & "2.00" & Chr(34) & ">"
...
'   Set rsTemp = cnSistema.Execute("Select Count(*) as Qte from NFe WHERE idNFe = " & Val(Mid(lvwNFes.SelectedItem.Key, 2, Len(lvwNFes.SelectedItem.Key))))
...
'      Set rsRamoAtividade = cnSistema.Execute("Select * From RamosAtividade WHERE idRamoAtividade=" & rsClientes!idRamoAtividade)
'      If rsRamoAtividade!idRamoAtividade = "Consumidor Final" Then
...
'Private Function Verifica_Aprovacao()
'Dim handle As Integer
'Dim Linha As String
'Dim bRetorno As Boolean
'
'   sEmpresaNFe = LerArquivoINI("NFe", "Empresa", CaminhoINI & "\System.ini")
'
'   If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe.txt") <> "" Then
'      ' Atualizar Chave da NFe
'      handle = FreeFile
'      Open "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe.txt" For Input As #handle
'
'      bRetorno = False
...
''         If Not IsNull(rsGerarXML!Protocolo) Then
''            If Trim(rsGerarXML!Protocolo) <> "" Then
''               cmdTransmitir.Enabled = False
''            Else
''               cmdTransmitir.Enabled = True
''            End If
...
Dim strMensagem As String
   ...
   Do While Not rsNFe.EOF
      ...
      ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
      sEmpresaNFe = LerArquivoINI("NFe", "Empresa", CaminhoINI & "\System.ini")
      If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & rsNFe!ChaveNFe & "-can.err") <> "" Then
         ' Atualizar Chave da NFe
         handle = FreeFile
         Open "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & rsNFe!ChaveNFe & "-can.err" For Input As #handle
         ...
         Kill "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & rsNFe!ChaveNFe & "-can.err"
      End If
...
'''      ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'''      sEmpresaNFe = LerArquivoINI("NFe", "Empresa", CaminhoINI & "\System.ini")
'''      If Dir("C:\NF-e\" & sEmpresaNFe & "\Enviados\Autorizados\" & Format(rsNFe!DataEmissao, "yyyymmdd") & "\" & rsNFe!ChaveNFe & "-can.xml") <> "" And rsNFe!Situacao <> 3 Then
'''         cnSistema.Execute "Update NFe set " & _
'''                  "Situacao = 3 " & _
'''                  "Where idNFe = " & rsNFe!idNFe
'''      End If
...
'''      ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'''      sEmpresaNFe = LerArquivoINI("NFe", "Empresa", CaminhoINI & "\System.ini")
'''      If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & rsNFe!ChaveNFe & "-can.xml") <> "" And rsNFe!Situacao <> 3 Then
'''      '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'''         handle = FreeFile
'''         Open "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & rsNFe!ChaveNFe & "-can.XML" For Input As #handle
'''
'''         Line Input #handle, Linha
'''
'''         Contador = 1
...
'''                     "Where idNFe = " & rsNFe!idNFe
'''
'''            FileCopy "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & rsNFe!ChaveNFe & "-can.XML", "C:\NF-e\" & sEmpresaNFe & "\Enviados\Autorizados\" & Format(rsNFe!DataEmissao, "yyyymmdd") & "\" & rsNFe!ChaveNFe & "-can.XML"
'''         End If
'''      End If
...
''''      ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
''''      sEmpresaNFe = LerArquivoINI("NFe", "Empresa", CaminhoINI & "\System.ini")
''''      If Dir("C:\NF-e\" & sEmpresaNFe & "\Enviados\Autorizados\" & Format(rsNFe!DataEmissao, "yyyymmdd") & "\" & rsNFe!ChaveNFe & "_110111_01-procEventoNFe.xml") <> "" And rsNFe!Situacao <> 3 Then
''''         cnSistema.Execute "Update NFe set " & _
''''                  "Situacao = 3 " & _
''''                  "Where idNFe = " & rsNFe!idNFe
''''      End If
      ...
      ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
      sEmpresaNFe = LerArquivoINI("NFe", "Empresa", CaminhoINI & "\System.ini")
      If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & rsNFe!ChaveNFe & "-ret-env-canc.xml") <> "" And rsNFe!Situacao <> 3 Then
      '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
         handle = FreeFile
         Open "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & rsNFe!ChaveNFe & "-ret-env-canc.XML" For Input As #handle
 ...
 ' Notas Inutilizadas
   ...
   Do While Not rsNFeInutilizadas.EOF
      ...
      If IsNull(rsNFeInutilizadas!ChaveNFe) Or Trim(rsNFeInutilizadas!ChaveNFe) = "" Then
         If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(Val(rsNFeInutilizadas!Numero), 12) & "-ret-gerar-chave.txt") <> "" Then
            handle = FreeFile
            Open "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(Val(rsNFeInutilizadas!Numero), 12) & "-ret-gerar-chave.txt" For Input As #handle
            Line Input #handle, Linha
            sChaveNFe = Linha
            Close #handle
      ...
      If IsNull(rsNFeInutilizadas!Protocolo) Or Trim(rsNFeInutilizadas!Protocolo) = "" Then
         sEmpresaNFe = LerArquivoINI("NFe", "Empresa", CaminhoINI & "\System.ini")
         If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & Trim(rsNFeInutilizadas!ChaveNFe) & "-inu.XML") <> "" Then
            handle = FreeFile
...
' arquivo=“\\meuservidor\c\pasta_xml\0102012.xml”
' pastaconfiguracao=“x:\unimake\uninfe”
' logotipo=”c:\imagens\logotipo_filial_001.jpg”
' visualizar = 0
' imprimir = 1
End Sub
...
''            Open "C:\NF-e\Notas\Notas.TXT" For Output As #1
''            Print #1, "tbAmb|" & LerArquivoINI("NFe", "Ambiente", CaminhoINI & "\System.ini")    ' 1 - Produção ou 2 - Homologação
''            Print #1, "chNFe|" & rsNFe!ChaveNFe
''            Print #1, "nProt|" & rsNFe!Protocolo
''            Print #1, "xJust|" & sMotivo
''            Close #1
''
''            sEmpresaNFe = LerArquivoINI("NFe", "Empresa", CaminhoINI & "\System.ini")
''            FileCopy "C:\NF-e\Notas\Notas.TXT", "C:\NF-e\" & sEmpresaNFe & "\Envio\" & Trim(rsGerarXML!ChaveNFe) & "-ped-can.txt"
''            Kill "C:\NF-e\Notas\Notas.TXT"
''         Else
''            MsgBox "O motivo é obrigatório", vbExclamation + vbOKOnly, "Campos Obrigatórios"
''         End If
...
'         Print #1, "tpAmb|" & LerArquivoINI("NFe", "Ambiente", CaminhoINI & "\System.ini") ' 1 - Produção ou 2 - Homologação
'         Print #1, "versao|3.10"
'         Print #1, "UF|" & rsUFs!Codigo
'         Print #1, "nNF|" & Trim(sNumeroInutilizar)
'         Print #1, "cNF|0"
'         Print #1, "serie|1"
'         Print #1, "AAMM|" & Format(Date, "yymm")
'         Print #1, "CNPJ|" & RemoveCaracteres(rsEmpresa!CNPJ_CPF)
         ...
         sEmpresaNFe = LerArquivoINI("NFe", "Empresa", CaminhoINI & "\System.ini")
'         FileCopy "C:\NF-e\Notas\Notas.TXT", "C:\NF-e\" & sEmpresaNFe & "\Envio\" & StrZero(Val(sNumeroInutilizar), 12) & "-gerar-chave.txt"
...
''   tmrAtualiza.Enabled = False
''   If MsgBox("Confirma Inutilizar Número da Nota", vbYesNo + vbQuestion, "Confirmação") = vbYes Then
''      sNumeroInicial = InputBox("Digite o número Inicial a inutilizar", "Inutilizar numeração", "")
''      sNumeroFinal = InputBox("Digite o número Final a inutilizar", "Inutilizar numeração", "")
'''      Set rsTemp = cnSistema.Execute("SELECT * FROM NFeInutilizadas WHERE Numero = " & Val(sNumeroInutilizar))
'''      If rsTemp.EOF Then
''         Set rsUFs = cnSistema.Execute("SELECT * FROM UFs WHERE idUF = " & rsEmpresa!idUF)
''
...
''         Print #1, "tpAmb|" & LerArquivoINI("NFe", "Ambiente", CaminhoINI & "\System.ini") ' 1 - Produção ou 2 - Homologação
''         Print #1, "cUF|" & rsUFs!Codigo
''         Print #1, "ano|" & Format(Date, "mm")
''         Print #1, "CNPJ|" & RemoveCaracteres(rsEmpresa!CNPJ_CPF)
''         Print #1, "mod|55"
''         Print #1, "serie|1"
''         Print #1, "nNFIni|" & Trim(sNumeroInicial)
''         Print #1, "nNFFin|" & Trim(sNumeroFinal)
''         Print #1, "xJust|Numeracao emitida incorretamente pelo funcionario"
''
''         Close #1
''
''         sEmpresaNFe = LerArquivoINI("NFe", "Empresa", CaminhoINI & "\System.ini")
''         FileCopy "C:\NF-e\Notas\Notas.TXT", "C:\NF-e\" & sEmpresaNFe & "\Envio\" & StrZero(Val(sNumeroInicial), 12) & "-ped-inu.txt"
''         Kill "C:\NF-e\Notas\Notas.TXT"
''
''         cnSistema.Execute "Insert Into NFeInutilizadas (ChaveNFe,Numero,Data,Protocolo,NumeroInicial,NumeroFinal) " & _
''                           "Values ('','" & Val(sNumeroInicial) & "','" & Date & "','','" & Val(sNumeroInicial) & "','" & Val(sNumeroFinal) & "')"
'''      Else
'''         MsgBox "Número já Inutilizado", vbExclamation + vbOKOnly, "Erro de NFe"
'''      End If
''   End If
...
Dim strMensagem As String
   ...
   If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe.txt") <> "" Then
      ...
      Wend
      Close #handle
      If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe.txt") <> "" Then FileCopy "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe.txt", "C:\NF-e\" & sEmpresaNFe & "\Temp\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe.txt"
      If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe-orig.txt") <> "" Then FileCopy "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe-orig.txt", "C:\NF-e\" & sEmpresaNFe & "\Temp\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe-orig.txt"
      If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe.txt") <> "" Then Kill "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe.txt"
      If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe.txt") <> "" Then Kill "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe-orig.txt"
   End If

   ' Verifica se XML teve erro de conversão
   ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
   sEmpresaNFe = LerArquivoINI("NFe", "Empresa", CaminhoINI & "\System.ini")
   If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe.err") <> "" Then
      ' Atualizar Chave da NFe
      handle = FreeFile
      Open "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe.err" For Input As #handle
      ...
      Kill "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & StrZero(rsNFe!Numero, 6) & "_" & RemoveCaracteres(rsEmpresa!CNPJ_CPF) & "_001_" & Format(rsNFe!DataEmissao, "dd_mm_yyyy") & "-nfe.err"
   End If
   
   ' Verifica se XML não possui erros
   ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
   sEmpresaNFe = LerArquivoINI("NFe", "Empresa", CaminhoINI & "\System.ini")
   If Dir("C:\NF-e\" & sEmpresaNFe & "\Retorno\" & rsNFe!ChaveNFe & "-nfe.err") <> "" Then
      ' Atualizar Chave da NFe
      handle = FreeFile
      Open "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & rsNFe!ChaveNFe & "-nfe.err" For Input As #handle
      ...
      Kill "C:\NF-e\" & sEmpresaNFe & "\Retorno\" & rsNFe!ChaveNFe & "-nfe.err"
   End If
End Sub
...
'            Next
            ...
            If sStatus <> "" Then
               Set ItemList = lvwMensagens.ListItems.Add(, "R" & CStr(IdMensagens), sStatus)
                   ItemList.SubItems(1) = xMotivo
...
LError:
    PassarErro Err, TypeName(Me) & ".NotasNFCe"
End Function

Private Function NotasNFCeGrupoB()
On Error GoTo LError
...
Dim dValorTributos As Double
   ...
   ' Define uma chave randomica para a nota
     Randomize
     Dim randomico As Double
     Dim sNF As String
     randomico = Int(Rnd * 99999999) + 1
     sNF = randomico
     ...
     Else
        sFinalidade = "2"                                                       ' 1 - NFe Normal / 2 - Complementar / 3 - Ajuste
     End If
     sProcessoEmissao = "1"                                                     ' Utilizando Software próprio
     ...
     Print #1, FTAG("cUF", sUF) & _
               FTAG("cNF", sNF) & _
               FTAG("natOp", sNaturezaOperacao) & _
               FTAG("mod", sModelo) & _
               FTAG("serie", sSerie) & _
               FTAG("nNF", sNumero) & _
               FTAG("dhEmi", sDataEmissao) & _
               FTAG("dhSaiEnt", sDataSaida) & _
               FTAG("tpNF", sTipoNF) & _
               FTAG("idDest", sidDest) & _
               FTAG("cMunFG", sCodigoMunicipio) & _
               FTAG("tpImp", sProcessoEmissao) & _
               FTAG("tpEmis", sTipoEmissao) & _
               FTAG("cDV", sDVChaveAcesso) & _
               FTAG("tpAmb", sidAmbiente) & _
               FTAG("finNFe", sFinalidade) & _
               FTAG("indFinal", sFinalidade) & _
               FTAG("indPres", sIndicadorPresenca) & _
               FTAG("procEmi", sProcessoEmissao) & _
               FTAG("verProc", sVersaoAplicativo)

     Print #1, "</ide>"

LError:
    PassarErro Err, TypeName(Me) & ".NotasNFCeGrupoB"
End Function

Private Function NotasNFCeGrupoC()
On Error GoTo LError
   ...
   '=======================================================================================================
     ...
     Print #1, FTAG("CNPJ", sECNPJ) & _
               FTAG("xNome", sERazaoSocial) & _
               FTAG("xFant", sEFantasia)
               
     Print #1, "<enderEmit>"
     Print #1, FTAG("xLgr", sELogradouro) & _
               FTAG("nro", sENumero) & _
               FTAG("xBairro", sEBairro) & _
               FTAG("cMun", sECodigoMunicipio) & _
               FTAG("xMun", sEMunicipio) & _
               FTAG("UF", sEUF) & _
               FTAG("CEP", sECEP) & _
               FTAG("cPais", sECodigoPais) & _
               FTAG("xPais", sEPais) & _
               FTAG("fone", sETelefone)
     Print #1, "</enderEmit>"
     
     Print #1, FTAG("IE", sEIE) & _
               FTAG("IM", sEIM) & _
               FTAG("CNAE", sECNAE) & _
               FTAG("CRT", sCRT)
...
LError:
    PassarErro Err, TypeName(Me) & ".NotasNFCeGrupoC"
End Function

Private Function NotasNFCeGrupoE()
On Error GoTo LError
    ...
    '=======================================================================================================
      ...
      Print #1, "<dest>"
      If Len(Trim(sDCNPJ)) > 11 Then
         Print #1, FTAG("CNPJ", sDCNPJ) & _
                   FTAG("xNome", sDRazaoSocial)
      Else
         Print #1, FTAG("CPF", sDCNPJ) & _
                   FTAG("xNome", sDRazaoSocial)
      End If
      
      Print #1, "<enderDest>"
      Print #1, FTAG("xLgr", sDLogradouro) & _
                FTAG("nro", sDNumero) & _
                FTAG("xBairro", sDBairro) & _
                FTAG("cMun", sDCodigoMunicipio) & _
                FTAG("xMun", sDMunicipio) & _
                FTAG("UF", sDUF) & _
                FTAG("CEP", sDCEP) & _
                FTAG("cPais", sDCodigoPais) & _
                FTAG("xPais", sDPais) & _
                FTAG("fone", sDTelefone)
      Print #1, "</enderDest>"
      
     Print #1, FTAG("indIEDest", sIndicadorIEDest) & _
               FTAG("IE", sDIE)
      Print #1, "</dest>"

LError:
    PassarErro Err, TypeName(Me) & ".NotasNFCeGrupoE"
End Function

Private Function NotasNFCeGrupoH()
On Error GoTo LError
...
Private Function FTAG(sTAG As String, sConteudo As String) As String
On Error GoTo LError

   FTAG = "<" & sTAG & ">" & sConteudo & "<\" & sTAG & ">" & vbCrLf
   
LError:
    PassarErro Err, TypeName(Me) & ".FTAG"
End Function

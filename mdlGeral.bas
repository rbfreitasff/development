...
Public I_MesAno As String              ' Retorna o MÍs e Ano da Folha Corrente
...
Public I_PrefixoTelefonico As String   ' Prefixo TelefÙnico
Public BancoDeDados As String          ' LocalizaÁ„o do Banco de Dados
...
Sub Main()
   If Len(CStr(Date)) < 10 Then
      ...
      If SetLocaleInfo(lngLocale, LOCALE_SSHORTDATE, "dd/MM/yyyy") = False Then
         MsgBox "Configure a Data para o Formato DD/MM/AAAA no Painel de Controle", vbInformation + vbOKOnly, "ConfiguraÁıes"
         End
      End If
   End If
...
'''''   cnSistema.Open
   ...
   ' Setar configuraÁıes da Empresa Emitente
...
'''''         MsgBox "N„o foi possÌvel gravar arquivo de configuraÁ„o" & Chr(13) & "Entre em contato com o suporte", vbInformation + vbOKOnly, "Sistema"
'''''         End
'''''      End If
...
'''''      MsgBox "N„o foi possÌvel iniciar o sistema" & Chr(13) & "Entre em contato com o suporte", vbInformation + vbOKOnly, "Sistema"
'''''      End
'''''   End If
...
Dim sCNPJ_CPF As String, _
   ...
   If Len(sCNPJ_CPF) <> 11 And Len(sCNPJ_CPF) <> 14 Then
      MsgBox "Tamanho do CNPJ/CPF est· incorreto", vbInformation + vbOKOnly, "InformaÁıes"
      CNPJ_CPF = "ERRO"
      Exit Function
   End If
   ...
   For Contador = 1 To 2
       ...
       iMultiplicador = 2                    ' Inicializa vari·vel para multiplicaÁ„o
       For i = 1 To iTamanho                 ' Multiplicar os n˙mero do CNPJ de traz para frente
           If iMultiplicador = iTeste Then   ' Se multiplicador for o mesmo que teste reinicializ·-la
              iMultiplicador = 2
           End If
           iSoma = iSoma + Val(Mid(sNumero, iTamanho - i + 1, 1)) * iMultiplicador ' Soma ser· igual a soma + posiÁ„o do n˙mero do CPF de traz para frente * o multiplicador
   ...
   If sCNPJ_CPF <> sNumero Then
      MsgBox "CNPJ/CPF est· incorreto", vbInformation + vbOKOnly, "InformaÁıes"
      CNPJ_CPF = "ERRO"
      Exit Function
   End If
...
' Validar o Tamanho
   ...
   If Not IsDate(sArg) Or Val(Mid(sArg, 7, 4)) < 1900 Then
      MsgBox "Data Inv·lida", vbOKOnly + vbInformation, "ValidaÁ„o"
      ValidarData = False
      Exit Function
   End If
...
Public Function ListarArquivos(ByVal Caminho As String) As String()
    'AtenÁ„o: FaÁa referÍncia ‡ biblioteca Micrsoft Scripting Runtime
...
LErro:
    ...
    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure ClonarRecordset of MÛdulo ModUtil"
...
Public Function Validar_Permissao(ByVal intStatus As Integer, strMenu As String) As Boolean
   ...
   If Mid(idAcesso, intStatus, 1) = 0 Then
      MsgBox "Usu·rio n„o autorizado para esta operaÁ„o", vbInformation, "Acesso Negado"

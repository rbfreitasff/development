...
Private Sub cmdOK_Click()
   If txtUsuario.text = Empty Then
      Beep
      MsgBox "Informe o Usuário para Entrar no Sistema", vbOKOnly + vbExclamation, "Login no Sistema"
      txtUsuario.SetFocus
   Else
      ' Limpar
...
''         MDISistema.Show
      Else
         ...
         If rsUsuarios.EOF Then
            Beep
            MsgBox "Usuário não Cadastrado, por favor confira", vbOKOnly + vbExclamation, "Login no Sistema"
            txtUsuario.SetFocus
         ...
         Else
            If Not UCase(rsUsuarios!Senha) = UCase(Trim(txtSenha.text)) Then
               Beep
               MsgBox "Senha não confere, por favor confira", vbOKOnly + vbExclamation, "Senha"
               txtSenha.SetFocus
               txtSenha.SelStart = 0
